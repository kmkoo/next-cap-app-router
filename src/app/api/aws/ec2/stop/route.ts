...
async function waitForStopped(instanceId: string, retries = 20, delay = 5000): Promise<boolean> {
  for (let i = 0; i < retries; i++) {
    const describe = new DescribeInstancesCommand({ InstanceIds: [instanceId] });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!success) {
      return NextResponse.json({ success: false, error: "중단 상태 확인 실패 (timeout)" }, { status: 500 });
    }
